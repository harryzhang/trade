...
<body>
	...
	<form id="loginForm" action="<c:url value='/member/saveTrans.html'/>" method="post">
	    <div class="exit-wrap">
	        <div class="item">
	        	...
	        	<input type="text"   readonly="readonly"    value="截止今天可转换额度：${pointAmt}" >
	        </div>
	        <div class="item">
    ...
    <script type="text/javascript">
        ...
        $("#submit").on('click', function(){
            ...
            var param = {"code":code , "pageId":"transJifeng","jifeng":jifeng,"token":$("#token").val()};
            submitLogin(param);
        });
        
        //提交
        function submitLogin(param){
        	$.post('<c:url value="/member/saveTransPoint.html"/>', param, function(data) {
        		...
        		if(data.result==1){
    				...
    				refCode(false);
    				HHN.popup("转换储值豆不能大于当前可转总储值豆");
    			}else if(data.result==6){ 
    				refCode(false);
    				HHN.popup("转换储值豆必须是50的倍数");
