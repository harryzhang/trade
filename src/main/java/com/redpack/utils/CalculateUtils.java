...
public class CalculateUtils {
	...
	/**
	 * 提供（相对）精确的除法运算。当发生除不尽的情况时，由scale参数指 定精度，以后的数字指定模式处理。
	 * 
	 * @param v1
	 *            被除数
	 * @param v2
	 ...
	 * 按占比分配金额
	 * @param baseVal      基数
	 * @param numeratorVal 分子
	 * @param denominatorVal 分母
	 * @return  
	 * @author: zhangyunhmf
	 * @date: 2014年9月29日上午9:55:31
	 */
	public static double splitByRate(double baseVal, double numeratorVal, double denominatorVal ){
	    if(eq(baseVal,0)) return 0;
	    if(eq(denominatorVal,0)) return 0;
	    if(eq(numeratorVal,0)) return 0;
	    return baseVal*(numeratorVal/denominatorVal);
	}
}
